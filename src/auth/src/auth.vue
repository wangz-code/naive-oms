...
import { useDialogPro, useMessagePro, useSkeleton, type AuthData, type OtherModalProps, type Uri } from '#/index';
import { debounce } from 'lodash-es';
import AuthItem from './auth-item.vue';
import { onMounted, reactive, ref } from 'vue';
import { NSkeleton, NCheckboxGroup, NTabs, NTabPane } from 'naive-ui';
...
const removeAuth = async (functionPath: string[]) => {
  ...
  messagePro.success(localeRef.value.removeConfirmSuccess);
  waitRefreshAuths();
};
...
const addAuth = async (functionPath: string[]) => {
  ...
  messagePro.success(localeRef.value.addSuccess);
  waitRefreshAuths();
};
