<template>
	<n-popover trigger="click" placement="bottom">
		...
		<VueDraggable ref="el" v-model="colsConfig" @update="refresh" :style="style.colsBox">
			<div v-for="item in colsConfig" :key="item.key">
				<n-flex v-if="item.title" class="m-t-2" align="center">
					<n-icon v-if="item.column.fixed" :component="PinnedOff" @click="fixedToggle(item.column)" :style="style.pointer" />
...
const uid = MD5(location.href).toString();
