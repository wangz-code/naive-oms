 ...
 */
export const useXModal = <P = any, T = ChooseOpts<P>>(xModalOpt: XModalOptions) => {
  const { localeRef } = useLocale('global');
  const { title = localeRef.value.choose, width = '400px', content, maskClosable = true, multiple, style = {} } = xModalOpt;
  let modalApi: ModalReactive;
  const modal = useModal();
  const open = (opt: T) => {
    const options = (opt || {}) as ChooseOpts<P>;
