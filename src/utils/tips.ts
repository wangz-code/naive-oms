...
export const useDialogPro = () => {
	...
	dialog.delConfirm = (opt: DialogOptions) => {
		dialog.warning({ title: localeRef.value.pleaseConfirm, content: localeRef.value.delete, positiveText: localeRef.value.confirmDelete, negativeText: localeRef.value.cancel, ...opt });
	};
	// 主要用于Api错误弹出框,如果后续有定制可以直接修改这里 NTag 组件使用 ()=>[内容]性能更好
	dialog.errorApi = ({ code, info }) => {
		dialog.error({
			title: localeRef.value.tip,
